...
const ListCard = ({props}) => {
  ...
  useEffect(()=>{
    const Movies=CardsData.filter(movie=>(//la filtrage selon l'age
      movie.age<=userAge
    ));
  ...
  return (
    <div>
      ...
      <div>
        ...
        <input type="text" placeholder="recherche" id="input" onChange={handleChange} required/>
        <button>
      ...
      <div>
        {
          filteredMoviecards.filter((value)=>{
            return(
              value.titre
              .toLocaleLowerCase()
          ...
          .map((element)=>{
          return (<div>
            <Link to="/MoviesListComponent"> <MovieCards cards={element} 
            handlefavoriteClick={AddFavoritesmovies}/ ></Link>
          </div>
          )
          })
        }
       ...
       <div>
       {
         ...
         .map((element)=>{
         return (<div>
           <Link to="/MoviesListComponent"> <MovieCards favorite={element} 
           handlefavoriteClick={AddFavoritesmovies}/ ></Link>
         </div>
         )
         })
